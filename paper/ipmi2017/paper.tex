...
\titlerunning{Hierarchical Region-Network Sparsity for
Brain Imaging}
...
% \cite{daudet2004sparse},
natural images %\cite{harzallah2009combining}
and
videos
%\cite{kang2015structured}
...
\cite{jenatton2012multi} to create a set of convex sparse penalty terms.
...
\begin{figure*}
  \vspace{-0.6cm}
  \centering
  \includegraphics[width=0.9\textwidth]{../figures/reg_net_prior_colin2.pdf}
  \vspace{-0.3cm}
...
Major brain networks already emerge in human individuals around birth
\cite{doria2010}.
In adults,
the regions that compose
a given cohesive network have more
...
that guide supervised model fitting.
...
%
As a consequence of this region-network sparsity tree,
...
%
Moreover,
the coefficient of all region or all network groups can
...
given a single brain image. Let the set \(\mathcal X\in\R^{n \times p}\) represent brain
...
various psychological tasks from brain maps.
}
\end{centering}
...
\subsection{Data}
% \paragraph{Data.}
As the currently biggest open-access dataset in brain imagingf5,
...
across participants
(see appendix for detailed descriptions of dataset and tasks).
...
\begin{figure*}
  \vspace{-0.6cm}
  \begin{centering}
    % \includegraphics[width=0.90\textwidth]{../figures/reg_net_ratio_colin2.pdf}
    \includegraphics[width=0.90\textwidth]{../figures/reg_net_ratio_colin3.png}
    \vspace{-0.3cm}
    ...
    %
    % Third,
    % equal emphasis on region versus network neighborhoods in the
    % high-dimensional neuroimaging data yields more dispersed,
